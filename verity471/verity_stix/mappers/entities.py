...
from verity471.verity_stix.sco import map_url, map_domain, map_ipv4, map_ipv6, map_email_address, \
    map_autonomous_system, map_file_hash, map_filename, map_crypto_wallet, map_user_account
from verity471.verity_stix.sdo import map_malware, map_threat_actor, map_vulnerability
...
class EntitiesMapper:
    """
    Map entities attached to the reports under `.entities` key
    """
    def __init__(self):
        self.mapper_configs = [
    ...
    def map(self, type: str, value: str, *args, **kwargs):  # noqa need `type` here as it comes from the API
        if len(value) < 2:
            log.info("Skipping entity. Value too short. type=`%s` value=`%s`", type, value)
            return
