...
class ChatRoomMessageStream(BaseModel):
    """
    Chats Message and room
    """ # noqa: E501
    chat_room: Optional[RoomStream] = None
    message: ChatMessageStream
    server: Optional[ServerStream] = None
    __properties: ClassVar[List[str]] = ["chat_room", "message", "server"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of chat_room
        if self.chat_room:
            _dict['chat_room'] = self.chat_room.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "chat_room": RoomStream.from_dict(obj["chat_room"]) if obj.get("chat_room") is not None else None,
            "message": ChatMessageStream.from_dict(obj["message"]) if obj.get("message") is not None else None,
            "server": ServerStream.from_dict(obj["server"]) if obj.get("server") is not None else None
        })
        return _obj
