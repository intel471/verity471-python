...
class GirsTreeResponse(BaseModel):
    """
    GirsTreeResponse
    """ # noqa: E501
    count: StrictInt = Field(description="The total count of all GIRs returned.")
    girs: List[GirTree] = Field(description="A list of GIRs organized in a hierarchical tree structure.")
    __properties: ClassVar[List[str]] = ["count", "girs"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in girs (list)
        _items = []
        if self.girs:
            for _item_girs in self.girs:
                if _item_girs:
                    _items.append(_item_girs.to_dict())
            _dict['girs'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "count": obj.get("count"),
            "girs": [GirTree.from_dict(_item) for _item in obj["girs"]] if obj.get("girs") is not None else None
        })
        return _obj
